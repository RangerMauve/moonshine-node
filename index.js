...
function listDevices() {
  try {
    ...
    console.error(
      `${YELLOW}Could not list audio devices. Are you on Linux with ALSA?${RESET}`
    );
...
async function main() {
  ...
  // Create transcriber
  const transcriber = new NodeMicTranscriber(
    modelURL,
    {
      onModelLoadStarted: () => {
        if (options.verbose)
          console.log(`${YELLOW}⏳ Loading models...${RESET}`);
      },
      onModelLoaded: () => {
        if (options.verbose) {
      ...
      onSpeechEnd: () => {
        if (options.verbose) {
          process.stdout.write(
            CLEAR_LINE + `${YELLOW}🤔 Processing...${RESET}`
          );
        }
      },
      ...
      onTranscriptionCommitted: (text) => {
        ...
        if (options.once && transcriptionCount >= 1) {
          if (options.verbose)
            console.log(`${YELLOW}Exiting after one transcription.${RESET}`);
          transcriber.destroy();
          process.exit(0);
        }
      },
