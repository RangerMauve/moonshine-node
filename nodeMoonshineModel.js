...
export class NodeMoonshineModel {
    ...
    constructor(inputModelURL, precision = 'quantized', verbose = false) {
        ...
        if (this.#verbose) console.log(`[NodeMoonshineModel] Created with modelURL = ${modelURL}`);
    }
    ...
    async #load() {
        if (!this.#isModelLoading && !this.isLoaded()) {
            ...
            try {
                ...
                if (this.#verbose) console.log('[NodeMoonshineModel] Model loaded successfully');
            } catch (error) {
                this.#isModelLoading = false;
                throw error;
            }
    ...
    isLoaded() {
        return (
            this.#model.encoder !== undefined && 
            this.#model.decoder !== undefined
        );
    }
    ...
    async generate(audio) {
        if (this.#verbose) console.log(`[NodeMoonshineModel.generate] Processing ${audio.length} samples (${(audio.length/16000).toFixed(2)}s)`);

        if (this.isLoaded()) {
            const t0 = performance.now();
            const maxLen = Math.trunc(audio.length / 16000 * 14);
            if (this.#verbose) console.log(`[NodeMoonshineModel.generate] maxLen: ${maxLen}`);
            ...
            if (this.#model.encoder.inputNames.includes('attention_mask')) {
                ...
                encoderAttentionMask = new ort.Tensor('int64', maskData, [1, audio.length]);
                Object.assign(encoderInput, {
                    attention_mask: encoderAttentionMask,
                });
            }

            if (this.#verbose) console.log(`[NodeMoonshineModel.generate] Running encoder...`);
            const encoderOutput = await this.#model.encoder.run(encoderInput);
            if (this.#verbose) console.log(`[NodeMoonshineModel.generate] Encoder output keys:`, Object.keys(encoderOutput));
            ...
            if (this.#verbose) console.log(`[NodeMoonshineModel.generate] Starting decoder loop...`);
            for (let i = 0; i < maxLen; i++) {
                ...
                if (nextToken == this.#eosTokenID) {
                    if (this.#verbose) console.log(`[NodeMoonshineModel.generate] EOS token reached at iteration ${i}`);
                    break;
                }
                inputIDs = [[nextToken]];
            ...
            if (this.#verbose) console.log(`[NodeMoonshineModel.generate] Tokens: ${tokens.join(' ')}, Result: "${result}"`);
            return result;
        } else {
